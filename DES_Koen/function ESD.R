...
##        DOI: 
## 
## This code illustrates how the ESD approach, as defined in the paper, can be implemented in a general
## function in R Statistical Software. Depending on specific case study needs, modifications to the 
## code may be required.
##
## The functions uses the 'fitdistcens' function, which is part of the 'fitdistrplus' package, to fit 
...
ESD <- function(data, events, dist) {
  ...
  fit <- sapply(events, function(e) {
    
    # Prepare the data according to the required format
    lr <- data.frame(left=data$time, right=ifelse(data$event==e, data$time, NA));
      
    # Estimate the distribution parameters
    fitdistcens(lr, dist)$estimate;
  
  }) # sapply
    
  
  ## Return the event-specific distribution parameters
  return(fit)
